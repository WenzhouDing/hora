...
class LeapHandGrasp(AllegroHandHora):
    ...
    def compute_reward(self, actions):
        ...
        # 1) All fingertips within 0.1m of object
        cond1 = (torch.sqrt(((obj_pos - finger_pos) ** 2).sum(-1)) < 0.1).all(-1)
        # 2) At least 2 fingers in contact with object
        ...
        if self._debug_counter % 50 == 0:
            ...
            print(f'[DEBUG] obj_z={obj_pos[0, -1, -1].item():.3f}')
            print(f'[DEBUG] finger dists: {finger_dists[0].cpu().numpy()}')
            print(f'[DEBUG] contacts={contact_condition[0].item():.0f}, cond1={cond1[0].item()}, cond2={cond2[0].item()}, cond3={cond3[0].item()}')
